...
enum TokenKind {
    ...
    // Assignment 2 Problem #3 reserve words.
    BREAK("break"), CASE("case"), CATCH("catch"), CONTINUE("continue"), DEFAULT("default"),
    DO("do"), DOUBLE("double"), FINALLY("finally"), FOR("for"), IMPLEMENTS("implements"), INTERFACE("interface"),
    ...
    // problem 3 operators
    //Bitwise operators.
    IOR("|"), XOR("^"), BAND("&"), BCOMP("~"), POUND("#"),
    // Shift operators.
    ALS("<<"), ARS(">>"), LRS(">>>"),
    // Assignment 2 problem# 2 operators.
    QUESTION("?"), COLON(":"), NOT_EQUAL("!="), DIV_ASSIGN("/="), MINUS_ASSIGN("-="), STAR_ASSIGN("*="),
    REM_ASSIGN("%="), ARS_ASSIGN(">>="), LRS_ASSIGN(">>>="), GT_ASSIGN(">="), ALS_ASSIGN("<<="), LT("<"),
    XOR_ASSIGN("^="), IOR_ASSIGN("|="), COR("||"), BAND_ASSIGN("&="),
    ...
    // Literals. adding here LONG and DOUBLE literal tokens.
    CHAR_LITERAL("<CHAR_LITERAL>"), FALSE("false"), INT_LITERAL("<INT_LITERAL>"), NULL("null"),
    STRING_LITERAL("<STRING_LITERAL>"), TRUE("true"), LONG_LITERL("<LONG_LITERAL>"), DOUBLE_LITERAL("<DOUBLE_LITERAL>");
